...
class Bdlm(base_model.BaseModel):
    ...
    def _tune(self, y, period, x=None, metric="mse", val_size=None, verbose=False):
        ...
        if x is not None:
            x_train, x_val = model_utils._train_val_split(x, val_size=val_size)
        ...
        for permutation in params_permutations:
            try:
                with warnings.catch_warnings():
                    ...
                    model = model + pydlm.trend(degree=permutation["trend"], discount=0.5, w=10)
                    model = model + pydlm.seasonality(period=self.period, discount=0.99, w=10)
                    model = model + pydlm.autoReg(degree=permutation["ar"], discount=0.99, w=10)
                    if x is not None:
                        for variable_id, x_variable in enumerate(x_train.T):
                            model = model + pydlm.dynamic(features=[[v] for v in x_variable], discount=0.99,
                                                          w=10, name=str(variable_id))
    ...
    def fit(self, y, period, x=None, metric="mse", val_size=None, verbose=False):
        ...
        self.model = self.model + pydlm.trend(degree=self.params["trend"], discount=0.5, w=10)
        self.model = self.model + pydlm.seasonality(period=self.period, discount=0.99, w=10)
        self.model = self.model + pydlm.autoReg(degree=self.params["ar"], discount=0.99, w=10)
        if x is not None:
